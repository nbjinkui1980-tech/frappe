...
spelled by nesting — `A and (B or C)` — which is the only spelling of it a
...
`#conjunction`. The slot stays, for restyling the cell — a host no longer needs
...
is the only one its new level has — the same thing taking the brackets off would
...
- anything, while `reorderable` is false. It turns off dragging between groups
  as well as within them — both are this one edit.
...
| --- | --- | --- |
| Assignment Rule | `safe_eval(cond, None, doc)` — the document **is** the locals | `status == "Open"`, no prefix |
...
operator with no rule — a legacy `timespan` — compiles to nothing rather than to
something that quietly changes what the rule matches. `timespan` is dropped on
...
- **`Group` has to be rendered inside the slot.** Vue resolves `inject` and slots
  ...
  throws — it is a group with no builder around it.
...
- **An empty template renders the default group**, as it does in every other
  slot here — Vue falls back when a slot produces no vnode. A node that draws
...
- **The row around the group is not the slot's.** The and/or cell, the drag
  ...
  component's; `#where`, `#conjunction` and `#actions` are how those are
  replaced.
...
fixable but not extendable — one failing validation, say — puts its own control
in `#addCondition` and disables that. The reach is the same, and what "cannot be
...
shows when there is no group to hang `#addCondition` on. A host blocking adds on
